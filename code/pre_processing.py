...
def prepare(data,posters_dict,label_dict,size):
	...
	for idx in posters_dict.keys():
  		try:
  			...
  			genre = data[data['imdbId']==int(idx)]['Genre'].values[0].split('|')
  			l = np.sum([np.eye(n_classes, dtype="uint8")[label_dict["genre2id"][s]] for s in genre], axis=0)
...
df =  pd.read_csv("../movie-genre-from-its-poster/MovieGenre.csv",encoding="ISO-8859-1")
#print(df.head());
...
for image in image_glob:
	try:
		posters_dict[get_id(image)] = imageio.imread(image) #Imageio.imread is similar to cv2.imread
		if cnt==17000:
		 	break
		print(cnt)
		cnt +=1;
